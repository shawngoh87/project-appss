...
function terminateParkingTime(theCar) {
    var terminateDuration;

    carRef.child(theCar).once('value').then(function (snapshot) {
        terminateDuration = snapshot.child('parking').child('duration').val();
    })

    terminateConfirmText =
            'Are you sure that you want to terminate the follwing parking?<br>' +
            'Car Plate Number&emsp;&emsp;&nbsp:' + theCar.toString() + '<br>' +
            'Duration &emsp;&emsp;&emsp;:' + timestamp2Time(terminateDuration).name + '<br>' +
            'Expected End Time is &emsp;:' + end_time_dis.getHours() + ' : ' + end_time_dis.getMinutes() + ' : ' + end_time_dis.getSeconds() + '<br><br>' +
    ...
    function loadPromocode() {
        ...
        firebase.database().ref(p_path).orderByKey().once('value').then(function (snapshot) {
            ...
            for (var eachPromotion in data) {
                ...
                } else if (p_status.toLowerCase() == 'expired') {
                    var str_all = '<div class="item-after" style = "color: red" >Expired</div>'
                } else if (p_status.toLowerCase() == 'used'){
                ...
                $$('.promo-list-all').append(str1 + eachPromotion + str2 + str_all + str3 + p_amount + str4 + p_expiry_date + str5 + p_text + str6);
            }
