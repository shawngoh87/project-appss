...
//Function to get active card address from latlng

function getActiveAddress(latlng,carPlate) {
    var geocoder = new google.maps.Geocoder;
    var address = { city: null, full_addr: null };
    geocoder.geocode({ 'location': latlng }, function (results, status) {
        if (status === 'OK') {
            if (results[0]) {
                ...
                address['city'] = city;
                address['full_addr'] = results[0].formatted_address;
            } else {
